...
%   F(x)=0. 
%
function [x1,mses,Tcpu,NF,NormF] = iRMILrpCGPM(A,b,lambda,true_x,method,model,para) 
...
for k=1:k_max
    ...
    if xu0 ~= xu0_old        
     ak = min(0.01,eat/(Normxkxk12));
     else
      ak = 0.01;   
    end 
    yuk = xu0+ak*(xu0-xu0_old);
    yvk = xv0+ak*(xv0-xv0_old);
    yk = yuk-yvk;
    Ayk = A(yk);
    tempy = AT(Ayk)-Atb;
    ...
    if k==1
        ...
        switch method
            ...
            case 'iRMILrp-CGPM'
             ...
             Normpk = sqrt(Normpk2);
             betak11u=Fyuk'*uuk/Normdk2;
             betak11v=Fyvk'*uvk/Normdk2;             
             betak2u=(NormFyk2-NormFyk/(Normpk)*Fyuk'*puk)/(NormFyk2+Normpk2);
             betak2v=(NormFyk2-NormFyk/(Normpk)*Fyvk'*pvk)/(NormFyk2+Normpk2);
             betak1u=max(0,betak11u);
             betak1v=max(0,betak11v);                        
                if k>=2 && Normuk <= 0.1*Normdk 
                     dku=-Fyuk+betak1u*dku ;
                     dkv=-Fyvk+betak1v*dkv ;
                else
                     dku=-1.5*Fyuk+betak2u*puk;
                     dkv=-1.5*Fyvk+betak2v*pvk;
                end
        end
    end
   ...
   if model==1
    ...
    elseif model==3
        ...
        NormFzk_new2 = Fzuk_new'*Fzuk_new+Fzvk_new'*Fzvk_new;
        NormFzk_new = sqrt(NormFzk_new2);
        eta_k=max(0.01,min(0.8, NormFzk_new));
        % check the Armijo-type line search condition
        while -Fzk_newtdk < sigma*t*eta_k*Normdk2/(1+Normdk2) && t>10^-6  
            % the Armijo-type line search condition violated
            t = t*tau;
            zuk_new = yuk+t*dku;
            zvk_new = yvk+t*dkv;
